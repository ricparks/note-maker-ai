...
export class NoteMakerCore {
	...
	private async processActiveMarkdown(
		...
		const subjectAny = subject.definition as {
			parseExistingNote?: (
	...
	private async processAdditionalMedia(
		...
		const { notesDir, photosDir } = this.resolveSubjectDirsAndLlm(subject);
	 ...
	 */
	private resolveSubjectDirsAndLlm(subject?: import("./subject").ActiveSubject): {
		notesDir: string;
		photosDir: string;
